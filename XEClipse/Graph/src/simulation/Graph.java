...
public class Graph implements Cloneable
{
   ...
   **/
   ...
   //**********************************************************
   public static void BreathFirstPrint(Graph g, int start)
   {
      boolean[ ] marked = new boolean [g.size( )];
      Object[ ] hold= new Object[g.size( ) ];
      int num=0;
      
      BreathFirstRecurse(g, start, marked, hold,num);
      
   }
   public static boolean check(Object[] Temp, Object it)
   {
	   ...
	   return false;///not sure but i think is false check. //b/c it got deleted by mistake dont remember.
   }
   public static void BreathFirstRecurse(Graph g, int v, boolean[ ] marked, Object[ ] hold,int num)
   {
      int[ ] connections = g.neighbors(v);
      int i;
      int nextNeighbor;
      int count=0;
       ...
       //setting the value equals true
      if (!marked[v])
      {
    	  System.out.print(g.getLabel(v)+" ");
    	  hold[num]=g.getLabel(v);
    	  marked[v] = true;
      }
      ...
      for (i = 0; i < connections.length; i++)
      {
    	  ...
    	  System.out.print(g.getLabel(connections[i])+" ");
